...
class TRP_Navigation_Based_on_Language{
    ...
    public function __construct() {
        ...
        define( 'TRP_NBL_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );
        define( 'TRP_NBL_PLUGIN_URL', plugin_dir_url( __FILE__ ) );
        ...
        require_once(  TRP_NBL_PLUGIN_DIR . 'includes/class-tp-nbl-walker-nav-menu.php' );
    ...
    function extra_fields( $item_id, $item, $depth, $args ) {
        ...
        <input type="hidden" name="trp-menu-filtering" value="<?php echo wp_create_nonce('trp-menu-filtering'); ?>"/>
        ...
        if( !empty( $all_languages ) && !empty( $our_languages ) ){
            ...
            <div class="trp-languages">
                <p class="description"><?php _e("Limit this menu item to the following languages", 'translatepress-multilingual'); ?></p>
                ...
                <label class="trp-language-checkbox-label">
                    ...
                    <?php _e( 'All Languages', 'translatepress-multilingual' ); ?>
                </label>
                    ...
                    if ( in_array($language_code, $our_languages) ) {
                        ...
                        <label class="trp-language-checkbox-label">
                            <input type="checkbox"
                                   ...
                                   name="trp-languages_<?php echo esc_attr($item->ID); ?>[]" <?php echo $readonly; ?> class="trp-nbol-lang-input"/>
    ...
    function update_menu($menu_id, $menu_item_db_id){
        ...
        if (!isset($_POST['trp-menu-filtering']) || !wp_verify_nonce($_POST['trp-menu-filtering'], 'trp-menu-filtering'))
            return;
    ...
    public function enqueue_navigation_script( ){
        ...
        if( $pagenow === 'nav-menus.php' ) {
            wp_enqueue_script('trp-sortable-languages', TRP_NBL_PLUGIN_URL . 'assets/js/trp-navigation.js', array('jquery'), TRP_PLUGIN_VERSION);
        }
    }
