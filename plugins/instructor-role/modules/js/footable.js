	...
	function ($, w, undefined) {
				...
				createDetail: function (element, data, createGroupedDetail, separatorChar, classes) {
					...
					for (var group in groups) {
						...
						for (var j = 0; j < groups[group].data.length; j++) {
							...
							element.append(
								$( '<div></div>' ).addClass( classes.detailInnerRow ).append(
									$( '<div></div>' ).addClass( classes.detailInnerName )
									.append( groups[group].data[j].name + separator )
								).append(
									$( '<div></div>' ).addClass( classes.detailInnerValue )
		...
		function Footable(t, o, id) {
			...
			ft.setColumnClasses = function () {
				...
				for (var c in ft.columns) {
					...
					if (col.className !== null) {
						...
						$.each(
							col.matches,
							function (m, match) { // support for colspans
								if ( ! first) {
									selector += ', ';
								}
			...
			ft.bindToggleSelectors = function () {
				...
				$table.find( opt.toggleSelector ).unbind( trg.toggleRow ).bind(
					trg.toggleRow,
					function (e) {
				...
				$table.find( opt.toggleSelector ).unbind( 'click.footable' ).bind(
					'click.footable',
					function (e) {
			...
			ft.redraw = function () {
				...
				.end()
				.find( '> tbody > tr.' + cls.detailShow ).each(
					function () {
						ft.createOrUpdateDetailRow( this );
					}
				);

				$table.find( "[data-bind-name]" ).each(
					function () {
						ft.toggleInput( this );
					}
				);

				$table.find( '> tbody > tr.' + cls.detailShow + ':visible' ).each(
					function () {
						var $next = $( this ).next();
			...
			ft.createOrUpdateDetailRow = function (actualRow) {
				...
				$row.find( '> td:hidden' ).each(
					function () {
						...
						if ( ! display) {
							display = $( this ).contents().clone( true, true );
						}
