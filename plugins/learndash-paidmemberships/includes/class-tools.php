...
class Learndash_Paidmemberships_Tools
{
    ...
    public function retroactive_access_process()
    {
        ...
        $trans_total = $wpdb->get_var( "SELECT COUNT(*) FROM {$pmpro_orders_table}" );
        $trans_percentage = ! empty( $trans_total ) ? ( $step - 1 ) / ceil( $trans_total / $per_batch ) * 100 : 100;
        
        $percentage  = $trans_percentage / 2;

        $query = $wpdb->prepare( "SELECT id, membership_id, status, gateway FROM {$pmpro_orders_table} LIMIT %d OFFSET %d", $per_batch, $offset );

        $orders = $wpdb->get_results( $query, OBJECT );

        foreach ( $orders as $order_db ) {
            $order = new MemberOrder( $order_db->id );

            if ( $order_db->gateway === 'free' || ( $order_db->gateway !== 'free' && $order_db->status === 'success' ) ) {
                Learndash_Paidmemberships::give_course_access_by_order( $order );
            } else {
                Learndash_Paidmemberships::remove_course_access_by_order( $order );
            }
        }
